import React, { useState, useEffect } from "react";
import axios from "axios";
import Swal from "sweetalert2";
import { Form, Button } from "react-bootstrap";
import "../style.css";
import Navigation from "../Acceuil/Navigation";
import Search from "../Acceuil/Search";
import TablePagination from '@mui/material/TablePagination';
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {faTrash, faFilePdf, faFileExcel, faPrint, faPlus,} from "@fortawesome/free-solid-svg-icons";
import * as XLSX from "xlsx";
import { jsPDF } from "jspdf";
import "jspdf-autotable";
import { createTheme, ThemeProvider } from "@mui/material/styles";
import Box from "@mui/material/Box";
import { Toolbar } from "@mui/material";
import {IoIosPersonAdd} from "react-icons/io";
// import ExportPdfButton from "./ExportPdfButton";
// import PrintList from "./PrintList";
const CompteList = () => {
  
    const [searchTerm, setSearchTerm] = useState("");
    const [filteredComptes, setFilteredComptes] = useState([]);
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(5);
    const [comptes, setComptes] = useState([]);
    const [user, setUser] = useState({});
    // const [users, setUsers] = useState([]);
    const [selectedItems, setSelectedItems] = useState([]);
    const [selectAll, setSelectAll] = useState(false);
    //-------------------edit-----------------------//
    const [editingCompte, setEditingCompte] = useState(null); // State to hold the compte being edited
    const [editingCompteId, setEditingCompteId] = useState(null);

    //---------------form-------------------//
    const [showForm, setShowForm] = useState(false);
    const [formData, setFormData] = useState({
        designations: "",
        type_compte: "",
        devise: "",
        rib: "",
        swift: "",
        adresse: "",
        remarque: "",

    });
    const [formContainerStyle, setFormContainerStyle] = useState({ right: "-500px", });
    const [tableContainerStyle, setTableContainerStyle] = useState({ marginRight: "0px", });
    const tableHeaderStyle = {
        background: "#f2f2f2",
        padding: "10px",
        textAlign: "left",
        borderBottom: "1px solid #ddd",
    };


    const fetchComptes = async () => {
      
            const response = await axios.get(
                "http://localhost:8000/api/comptes");
            setComptes(response.data.comptes);
            console.log("response",response);
            
    };

    useEffect(() => {
        fetchComptes();
    }, []);

    useEffect(() => {
        const filtered =  comptes&&comptes.filter((compte) =>
            compte.designations
                .toLowerCase()
                .includes(searchTerm.toLowerCase())
        );
        setFilteredComptes(filtered);
    }, [comptes, searchTerm]);

    const handleSearch = (term) => {
        setSearchTerm(term);
    };

    const handleChangePage = (event, newPage) => {
        setPage(newPage);
    };

    const handleCheckboxChange = (itemId) => {
        if (selectedItems.includes(itemId)) {
            setSelectedItems(selectedItems.filter((id) => id !== itemId));
        } else {
            setSelectedItems([...selectedItems, itemId]);
        }
    };

    const handleChangeRowsPerPage = (event) => {
        setRowsPerPage(parseInt(event.target.value, 10));
        setPage(0);
    };
    //------------------------- compte Delete Selected ---------------------//

    const handleDeleteSelected = () => {
        Swal.fire({
            title: 'Êtes-vous sûr de vouloir supprimer ?',
            showDenyButton: true,
            showCancelButton: false,
            confirmButtonText: 'Oui',
            denyButtonText: 'Non',
            customClass: {
                actions: 'my-actions',
                cancelButton: 'order-1 right-gap',
                confirmButton: 'order-2',
                denyButton: 'order-3',
            },
        }).then((result) => {
            if (result.isConfirmed) {
                selectedItems.forEach((id) => {
                    axios
                        .delete(`http://localhost:8000/api/comptes/${id}`)
                        .then((response) => {
                            fetchComptes();
                            Swal.fire({
                                icon: "success",
                                title: "Succès!",
                                text: 'compte supprimé avec succès.',
                            });
                        })
                        .catch((error) => {
                            console.error(
                                "Erreur lors de la suppression du compte:", error);
                            Swal.fire({
                                icon: "error",
                                title: "Erreur!",
                                text: 'Échec de la suppression du compte.',
                            });
                        });
                });
            }
        });

        setSelectedItems([]);
    };

    const handleChange = (e) => {
        setFormData({ ...formData, [e.target.name]: e.target.value });
    };

    const handleSelectAllChange = () => {
        setSelectAll(!selectAll);
        if (selectAll) {
            setSelectedItems([]);
        } else {
            setSelectedItems(comptes.map((compte) => compte.id));
        }
    };
    //------------------------- fournisseur print ---------------------//

    // const printList = (tableId, title, compteList) => {
    //     const printWindow = window.open(" ", "_blank", " ");
    //
    //     if (printWindow) {
    //         const tableToPrint = document.getElementById(tableId);
    //
    //         if (tableToPrint) {
    //             const newWindowDocument = printWindow.document;
    //             newWindowDocument.write(`
    //         <!DOCTYPE html>
    //         <html lang="fr">
    //         <head>
    //           <meta charset="UTF-8">
    //           <meta name="viewport" content="width=device-width, initial-scale=1.0">
    //           <h1 class="h1"> Gestion Commandes </h1>
    //           <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css">
    //           <style>
    //
    //             body {
    //               font-family: 'Arial', sans-serif;
    //               margin-bottom: 60px;
    //             }
    //
    //             .page-header {
    //               text-align: center;
    //               font-size: 24px;
    //               margin-bottom: 20px;
    //             }
    //
    //             .h1 {
    //               text-align: center;
    //             }
    //
    //             .list-title {
    //               font-size: 18px;
    //               margin-bottom: 10px;
    //             }
    //
    //             .header {
    //               font-size: 16px;
    //               margin-bottom: 10px;
    //             }
    //
    //             table {
    //               width: 100%;
    //               border-collapse: collapse;
    //               margin-bottom: 20px;
    //             }
    //
    //             th, td {
    //               border: 1px solid #ddd;
    //               padding: 8px;
    //               text-align: left;
    //             }
    //
    //             .footer {
    //               position: fixed;
    //               bottom: 0;
    //               width: 100%;
    //               text-align: center;
    //               font-size: 14px;
    //               margin-top: 30px;
    //               background-color: #fff;
    //             }
    //
    //             @media print {
    //               .footer {
    //                 position: fixed;
    //                 bottom: 0;
    //               }
    //
    //               body {
    //                 margin-bottom: 0;
    //               }
    //               .no-print {
    //                 display: none;
    //               }
    //             }
    //
    //             .content-wrapper {
    //               margin-bottom: 100px;
    //             }
    //
    //             .extra-space {
    //               margin-bottom: 30px;
    //             }
    //           </style>
    //         </head>
    //         <body>
    //           <div class="page-header print-no-date">${title}</div>
    //           <ul>
    //             ${Array.isArray(compteList)
    //                 ? compteList.map((item) => `<li>${item}</li>`).join("")
    //                 : ""
    //             }
    //           </ul>
    //           <div class="content-wrapper">
    //             ${tableToPrint.outerHTML}
    //           </div>
    //           <script>
    //             setTimeout(() => {
    //               window.print();
    //               window.onafterprint = function () {
    //                 window.close();
    //               };
    //             }, 1000);
    //           </script>
    //         </body>
    //         </html>
    //       `);
    //
    //             newWindowDocument.close();
    //         } else {
    //             console.error(`Table with ID '${tableId}' not found.`);
    //         }
    //     } else {
    //         console.error("Error opening print window.");
    //     }
    // };
    //------------------------- fournisseur export to pdf ---------------------//

    // const exportToPdf = () => {
    //     const pdf = new jsPDF();
    //
    //     // Define the columns and rows for the table
    //     const columns = [
    //         "Client",
    //         "Sujet",
    //         "Date de Réclamation",
    //         "Status de Réclamation",
    //         "Traitement de Réclamation",
    //         "Date de Traitement",
    //     ];
    //     const selectedRecouvrements = comptes.filter((compte) =>
    //         selectedItems.includes(compte.numero)
    //     );
    //     const rows = selectedRecouvrements.map((compte) => [
    //         compte.client_id,
    //         compte.sujet,
    //         compte.date_compte,
    //         compte.status_compte,
    //         compte.traitement_compte,
    //         compte.date_traitement,
    //     ]);
    //
    //     // Set the margin and padding
    //     const margin = 10;
    //     const padding = 5;
    //
    //     // Calculate the width of the columns
    //     const columnWidths = columns.map(
    //         (col) => pdf.getStringUnitWidth(col) * 5 + padding * 2
    //     );
    //     const tableWidth = columnWidths.reduce((total, width) => total + width, 0);
    //
    //     // Calculate the height of the rows
    //     const rowHeight = 10;
    //     const tableHeight = rows.length * rowHeight;
    //
    //     // Set the table position
    //     const startX = (pdf.internal.pageSize.width - tableWidth) / 2;
    //     const startY = margin;
    //
    //     // Add the table headers
    //     pdf.setFont("helvetica", "bold");
    //     pdf.setFillColor(200, 220, 255);
    //     pdf.rect(startX, startY, tableWidth, rowHeight, "F");
    //     pdf.autoTable({
    //         head: [columns],
    //         startY: startY + padding,
    //         styles: {
    //             fillColor: [200, 220, 255],
    //             textColor: [0, 0, 0],
    //             fontSize: 10,
    //         },
    //         columnStyles: {
    //             0: { cellWidth: columnWidths[0] },
    //             1: { cellWidth: columnWidths[1] },
    //             2: { cellWidth: columnWidths[2] },
    //             3: { cellWidth: columnWidths[3] },
    //             4: { cellWidth: columnWidths[4] },
    //             5: { cellWidth: columnWidths[5] },
    //             6: { cellWidth: columnWidths[6] },
    //             7: { cellWidth: columnWidths[7] },
    //         },
    //     });
    //
    //     // Add the table rows
    //     pdf.setFont("helvetica", "");
    //     pdf.autoTable({
    //         body: rows,
    //         startY: startY + rowHeight + padding * 2,
    //         styles: { fontSize: 8 },
    //         columnStyles: {
    //             0: { cellWidth: columnWidths[0] },
    //             1: { cellWidth: columnWidths[1] },
    //             2: { cellWidth: columnWidths[2] },
    //             3: { cellWidth: columnWidths[3] },
    //             4: { cellWidth: columnWidths[4] },
    //             5: { cellWidth: columnWidths[5] },
    //             6: { cellWidth: columnWidths[6] },
    //             7: { cellWidth: columnWidths[7] },
    //         },
    //     });
    //
    //     // Save the PDF
    //     pdf.save("comptes.pdf");
    // };
    //------------------------- fournisseur export to excel ---------------------//

    // const exportToExcel = () => {
    //     const selectedRecouvrements = comptes.filter((compte) =>
    //         selectedItems.includes(compte.id)
    //     );
    //     const ws = XLSX.utils.json_to_sheet(selectedRecouvrements);
    //     const wb = XLSX.utils.book_new();
    //     XLSX.utils.book_append_sheet(wb, ws, "Recouvrements");
    //     XLSX.writeFile(wb, "recouvrements.xlsx");
    // };

    //------------------------- compte Delete---------------------//
    const handleDelete = (id) => {
        Swal.fire({
            title: 'Êtes-vous sûr de vouloir supprimer ce compte ?',
            showDenyButton: true,
            showCancelButton: false,
            confirmButtonText: 'Oui',
            denyButtonText: 'Non',
            customClass: {
                actions: 'my-actions',
                cancelButton: 'order-1 right-gap',
                confirmButton: 'order-2',
                denyButton: 'order-3',
            },
        }).then((result) => {
            if (result.isConfirmed) {
                axios
                    .delete(`http://localhost:8000/api/comptes/${id}`)
                    .then((response) => {
                        if (response.data) {
                            // Successful deletion
                            fetchComptes();
                            Swal.fire({
                                icon: 'success',
                                title: 'Succès!',
                                text: "compte supprimé avec succès",
                            });
                        } else if (response.data.error) {
                            // Error occurred
                            if (response.data.error.includes("Impossible de supprimer ou de mettre à jour une ligne parent : une contrainte de clé étrangère échoue")) {
                                // Violated integrity constraint error
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Erreur!',
                                    text: "Impossible de supprimer le compte car il a des produits associés.",
                                });
                            }
                        }
                    })
                    .catch((error) => {
                        // Request error
                        console.error("Erreur lors de la suppression du compte:", error);
                        Swal.fire({
                            icon: 'error',
                            title: 'Erreur!',
                            text: `Échec de la suppression du compte. Veuillez consulter la console pour plus d'informations.`,
                        });
                    });
            } else {
                console.log("Suppression annulée");
            }
        });
    }
    //------------------------- compte EDIT---------------------//

    const handleEdit = (comptes) => {
        setEditingCompte(comptes); // Set the recouvrements to be edited
        // Populate form data with recouvrements details
        setFormData({
            designations: comptes.designations,
            type_compte: comptes.type_compte,
            devise: comptes.devise,
            rib: comptes.rib,
            swift: comptes.swift,
            adresse: comptes.adresse,
            remarque:comptes.remarque,
        });
        if (formContainerStyle.right === '-500px') {
            setFormContainerStyle({ right: '0' });
            setTableContainerStyle({ marginRight: '500px' });
        } else {
            closeForm();
        }
        // Show form
        // setShowForm(true);
    };
    useEffect(() => {
        if (editingCompteId !== null) {
            setFormContainerStyle({ right: '0' });
            setTableContainerStyle({ marginRight: '500px' });
        }
    }, [editingCompteId]);

    //------------------------- compte SUBMIT---------------------//

    useEffect(() => {
        fetchComptes();
    }, []);

    const handleSubmit =  (e) => {
        e.preventDefault();

        const url = editingCompte ? `http://localhost:8000/api/comptes/${editingCompte.id}` : 'http://localhost:8000/api/comptes';
        const method = editingCompte ? 'put' : 'post';
        axios({
            method: method,
            url: url,
            data: formData,
        }).then(() => {
            fetchComptes();
            Swal.fire({
                icon: 'success',
                title: 'Succès!',
                text: `compte ${editingCompte ? 'modifié' : 'ajouté'} avec succès.`,
            });
            setFormData({
                designations: '',
                type_compte: '',
                devise: '',
                rib: '',
                swift: '',
                adresse: '',
                remarque:'',
            });
            setEditingCompte(null); // Clear editing compte
            // closeForm();
        })
            .catch((error) => {
                console.error(`Erreur lors de ${editingCompte ? 'la modification' : "l'ajout"} du compte:`, error);
                Swal.fire({
                    icon: 'error',
                    title: 'Erreur!',
                    text: `Échec de ${editingCompte ? 'la modification' : "l'ajout"} du compte.`,
                });
            });
        if (formContainerStyle.right === "-500px") {
            setFormContainerStyle({ right: "0" });
            setTableContainerStyle({ marginRight: "500px" });
        } else {
            closeForm();
        }
    };

    //------------------------- compte FORM---------------------//

    const handleShowFormButtonClick = () => {
        if (formContainerStyle.right === '-500px') {
            setFormContainerStyle({ right: '0' });
            setTableContainerStyle({ marginRight: '500px' });
        } else {
            closeForm();
        }
    };

    const closeForm = () => {
        setFormContainerStyle({ right: '-500px' });
        setTableContainerStyle({ marginRight: '0' });
        setShowForm(false); // Hide the form
        setFormData({ // Clear form data
            designations: '',
            type_compte: '',
            devise: '',
            rib: '',
            swift: '',
            adresse: '',
            remarque:'',
        });
        setEditingCompte(null); // Clear editing compte
    };

    return (
        <ThemeProvider theme={createTheme()}>
            <Box sx={{ display: 'flex' }}>
                <Navigation />
                <Box component="main" sx={{ flexGrow: 1, p: 3, mt: 4 }}>
                    <Toolbar />

                    <div>
                        <h3>Liste des Comptes</h3>
                        <div className="search-container d-flex flex-row-reverse mb-3">
                            <Search onSearch={handleSearch} />
                        </div>
                        <Button
                            id="showFormButton"
                            onClick={handleShowFormButtonClick}
                            style={{ backgroundColor: 'white', color: 'black' }}
                        >
                            <IoIosPersonAdd  style={{ fontSize: '24px' }} />
                            {/*{showForm ? "Modifier le formulaire" : <IoIosPersonAdd />}*/}
                        </Button>
                        <div className="d-flex flex-row justify-content-end">
                            <div className="btn-group col-2">
                                {/*<PrintList*/}
                                {/*    tableId="compteTable"*/}
                                {/*    title="Liste des reclatioms"*/}
                                {/*    compteList={comptes}*/}
                                {/*    filtredcomptes={filteredComptes}*/}
                                {/*/>*/}

                                {/*<ExportPdfButton*/}
                                {/*    comptes={comptes}*/}
                                {/*    selectedItems={selectedItems}*/}
                                {/*/>*/}
                                {/*<Button className="btn btn-success btn-sm ml-2" onClick={exportToExcel}>*/}
                                {/*    <FontAwesomeIcon icon={faFileExcel} />*/}
                                {/*</Button>*/}
                            </div>
                        </div>
                        <div id="formContainer" className="mt-2" style={formContainerStyle}>
                            <Form className="col row" onSubmit={handleSubmit}>
                                <Form.Label className="text-center m-2"><h5>{editingCompte ? 'Modifier compte' : 'Ajouter un compte'}</h5></Form.Label>

                                <Form.Group className="col-sm-10 m-2">
                                    <Form.Label>Designation</Form.Label>
                                    <Form.Control type="text" placeholder="designation" name="designations" value={formData.designations} onChange={handleChange} />
                                </Form.Group>
                                <Form.Group className="col-sm-5 m-2">
                                    <Form.Label>Type compte</Form.Label>
                                    <Form.Control type="text" placeholder="type compte" name="type_compte" value={formData.type_compte} onChange={handleChange} />
                                </Form.Group>
                                <Form.Group className="col-sm-5 m-2">
                                    <Form.Label>Devise</Form.Label>
                                    <Form.Control type="text" name="devise" value={formData.devise} onChange={handleChange}>

                                    </Form.Control>
                                </Form.Group>
                                <Form.Group className="col-sm-4 m-2">
                                    <Form.Label>RIB</Form.Label>
                                    <Form.Control type="text" placeholder="rib" name="rib" value={formData.rib} onChange={handleChange} />
                                </Form.Group>
                                <Form.Group className="col-sm-4 m-2">
                                    <Form.Label>SWIFT</Form.Label>
                                    <Form.Control type="text" placeholder="swift" name="swift" value={formData.swift} onChange={handleChange} />
                                </Form.Group>
                                <Form.Group className="col-sm-10 m-2">
                                    <Form.Label>Adresse</Form.Label>
                                    <Form.Control type="text" placeholder="adresse" name="adresse" value={formData.adresse} onChange={handleChange} />
                                </Form.Group>
                                <Form.Group className="col-sm-10 m-2">
                                    <Form.Label>Remarque</Form.Label>
                                    <Form.Control type="text" placeholder="Remarque" name="remarque" value={formData.remarque} onChange={handleChange} />
                                </Form.Group>
                                <Form.Group className="col m-3 text-center">
                                    <Button type="submit" className="btn btn-success col-6">
                                        {editingCompte ? 'Modifier' : 'Ajouter'}
                                    </Button>
                                    <Button className="btn btn-secondary col-5 offset-1" onClick={closeForm}>Annuler</Button>
                                </Form.Group>
                            </Form>
                        </div>

                        <div id="tableContainer" className="table-responsive-sm" style={tableContainerStyle}>
                            <table className="table table-responsive table-bordered " id="compteTable">
                                <thead >
                                <tr>
                                    <th style={tableHeaderStyle}>
                                        <input type="checkbox" onChange={handleSelectAllChange} />
                                    </th>
                                    <th style={tableHeaderStyle}>Designation</th>
                                    <th style={tableHeaderStyle}>Type compte</th>
                                    <th style={tableHeaderStyle}>Devise</th>
                                    <th style={tableHeaderStyle}>RIB</th>
                                    <th style={tableHeaderStyle}>SWIFT</th>
                                    <th style={tableHeaderStyle}>Adresse</th>
                                    <th style={tableHeaderStyle}>Remarque</th>
                                    <th style={tableHeaderStyle}>Action</th>
                                </tr>
                                </thead>
                                <tbody>
                                {filteredComptes && filteredComptes.slice (page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((comptes) => (

                                    <tr key={comptes.id}>
                                        <td>
                                            <input type="checkbox" onChange={() => handleCheckboxChange(comptes.id)} checked={selectedItems.includes(comptes.id)} />
                                        </td>

                                        <td>{comptes.designations}</td>
                                        <td>{comptes.type_compte}</td>
                                        <td>{comptes.devise}</td>
                                        <td>{comptes.rib}</td>
                                        <td>{comptes.swift}</td>
                                        <td>{comptes.adresse}</td>
                                        <td>{comptes.remarque}</td>
                                        <td className="d-inline-flex">
                                            <Button className="btn btn-sm btn-info m-1" onClick={() => handleEdit(comptes)}>
                                                <i className="fas fa-edit"></i>
                                            </Button>
                                            <Button className="btn btn-danger btn-sm m-1" onClick={() => handleDelete(comptes.id)}>
                                                <FontAwesomeIcon icon={faTrash} />
                                            </Button>
                                        </td>
                                    </tr>
                                ))}

                                </tbody>
                            </table>



                            <Button className="btn btn-danger btn-sm" onClick={handleDeleteSelected}>
                                <FontAwesomeIcon icon={faTrash} /></Button>

                            <TablePagination
                                rowsPerPageOptions={[5, 10, 25]}
                                component="div"
                                count={filteredComptes.length}
                                rowsPerPage={rowsPerPage}
                                page={page}
                                onPageChange={handleChangePage}
                                onRowsPerPageChange={handleChangeRowsPerPage}
                            />
                        </div>
                    </div>

                </Box>
            </Box>
        </ThemeProvider>
    );
};

export default CompteList;